	...
	public static boolean readPacketInternal() throws IOException {
		...
		if (ServerProt.REFLECTION_CHEAT_CHECK == packet) {
			...
			if (packet == ServerProt.IF_OPENTOP) {
				...
				if (Static378.setVerifyId(local228)) {
					if (local220 == 2) {
						Static90.method5953();
					}
			...
			if (packet == ServerProt.PACKET_48) {
				...
				if (Static378.setVerifyId(local74)) {
				...
				if (Static378.setVerifyId(local220)) {
					Camera.reset();
				}
				packet = null;
				return true;
				...
				if (packet == ServerProt.PACKET_66) {
					...
					if (ServerProt.PACKET_0 == packet) {
						...
						if (packet == ServerProt.WIDGETSTRUCT_SETTING) {
							...
							if (Static378.setVerifyId(worldId)) {
								for (local506 = local220; local506 <= local228; local506++) {
									...
									Static327.properties.put(local1118, local1101);
								}
							}
							packet = null;
							return true;
							...
							if (ServerProt.UPDATE_FRIENDLIST == packet) {
								...
								if (packet == ServerProt.PACKET_55) {
									...
									if (packet == ServerProt.PACKET_35) {
										...
										local228 = inboundBuffer.g2();
										worldId = inboundBuffer.g2();
										if (Static378.setVerifyId(local220) && Static26.aClass161ArrayArray1[local74] != null) {
										...
										for (@Pc(2839) Class2_Sub32 local2839 = (Class2_Sub32) Static120.objStacks.head(); local2839 != null; local2839 = (Class2_Sub32) Static120.objStacks.next()) {
											...
											worldId = (int) (local2839.key >> 28 & 0x3L);
										...
										worldId = inboundBuffer.g2add();
										if (worldId == 65535) {
											worldId = -1;
										}
										local497 = inboundBuffer.ig2();
										if (Static378.setVerifyId(local497)) {
								...
								while (true) {
									do {
										...
										if (local1416 == null) {
											...
											if (Static1.verifyIdChanged) {
												method2432();
												Static1.verifyIdChanged = false;
											}
											try {
