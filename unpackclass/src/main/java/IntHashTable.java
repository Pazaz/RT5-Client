...
public final class IntHashTable {
	...
	public IntHashTable(@OriginalArg(0) int[] arg0) {
		...
		for (@Pc(28) int local28 = 0; local28 < local5 + local5; local28++) {
			this.buckets[local28] = -1;
		}
		@Pc(43) int local43 = 0;
		while (local43 < arg0.length) {
			@Pc(53) int local53;
			for (local53 = arg0[local43] & local5 - 1; this.buckets[local53 + local53 + 1] != -1; local53 = local53 + 1 & local5 - 1) {
			}
			this.buckets[local53 + local53] = arg0[local43];
			this.buckets[local53 + local53 + 1] = local43++;
		}
	}
}
